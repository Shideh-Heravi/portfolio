...
let fitty = (w => {
  ...
  const calculateMax = f => () => {

    computeStyle(f);

     let val = (f.element.parentNode.clientWidth / f.element.scrollWidth) * f.currentFontSize;      
  ...
  function fittyCreate(elements, options) {
    ...
    Object.assign(fittyOptions, defaultOptions); // expand default options
    Object.assign(fittyOptions, options); // override with custom options
    ...
    const publicFitties = elements.map(element => {
      ...
      Object.assign(f, fittyOptions); // defaults
      ...
      return {
        element,
        calculateMax : calculateMax(f),
        fit          : fit(f, DrawState.DIRTY),
        fitMax       : function(maxSize) { fitMax(f, DrawState.DIRTY, maxSize)() },
  ...
  function fitty(target, options = {}) {
    ...
    return typeof target === 'string' ?
      ...
      fittyCreate(document.querySelectorAll(target), options) :
